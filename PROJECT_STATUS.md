...
| Fair baseline + evaluation | 15 | 15 | Same-evidence B1 ran 3×40, replays offline, records cost/stability, compares with ProofTrail and includes a no-temporal ablation |
| Live agent + frozen dataset | 20 | 20 | Real provider adapter plus 40 frozen, replayable traces with recorded usage |
| Verified benchmark | 10 | 10 | Human-approved labels, family-balanced results and failure analysis |
...
## Completed milestone: 85 → 95

- A named human reviewer decided all 40 cases: 33 approved, 7 amended, none
  abstained, with 298 measured active minutes across two sittings.
- `review verify --require-complete` passes with zero invalid or stale records.
- The same offline report now resolves accepted human truth and writes
  `comparison.verified.{json,md}` with `headline_eligible: true`.
- Verified metrics and failure analysis are reflected in README,
  `docs/SUBMISSION_REPORT.md`, `docs/HUMAN_REVIEW_RESULTS.md` and
  `docs/TRAJECTORIES.md`.

Final submission media is the separate 95 → 100 step.

### Closing branch `feat/final-submission` — gates that move the score

The verified benchmark gate is complete. The final five points remain tied to
the recorded submission experience.
